...
pub fn parse_size(size_str: &str) -> Result<u64, String> {
    ...
    let num_part: String = clean_str.chars().take_while(|c| c.is_digit(10) || *c == '.').collect();
    let unit_part: String = clean_str.chars().skip(num_part.len()).filter(|c| !c.is_whitespace()).collect();
    
    if num_part.is_empty() {
        return Err(format!("Error! Invalid size format: {}", size_str));
    }
    
    let value: f64 = num_part.parse().map_err(|e| format!("Erorr! The number could not be deciphered: {}", e))?;
...
pub fn validate_inputs(file_path: &str, add: &Option<String>, size: &Option<String>) -> Result<(), String> {
